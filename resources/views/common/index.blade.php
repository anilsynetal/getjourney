...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between pb-0">
                <h5 class="card-title"> {{ $common_data['active_page'] }}</h5>
                ...
                @if ($common_data['is_add'])
                    @if ($common_data['is_modal'])
                        @php
                            $modal_class = $common_data['is_modal_large'] ? 'loadRecordModalLarge' : 'loadRecordModal';
                        @endphp
                        <button type="button" class="btn btn-outline-primary {{ $modal_class }}"
            ...
            <div class="card-body">
                ...
                @if (isset($common_data['is_status_filter']) && $common_data['is_status_filter'] == true)
                    <div class="row">
                        <div class="col-md-12">
    ...
    <div class="modal fade" id="commonModalQuick" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
    ...
    <div class="modal fade" id="commonModalLargeQuick" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
        aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
...
@section('script')
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('shown.bs.modal', '#commonModal, #commonModalLarge', function() {
                ...
                const country_code_modal = document.querySelector("#country_code");
                const phoneInputModalFieldModal = document.querySelector("#mobile");
                if (phoneInputModalFieldModal != undefined) {
                    const phoneInputModal = window.intlTelInput(phoneInputModalFieldModal, {
                        utilsScript: "{{ asset('assets/js/utils.js') }}",
                    });
                    phoneInputModal.setCountry('in');
                    $(document).ready(function() {
                        $(document).on('change', '#mobile', function() {
                            const countryCode = phoneInputModal.getSelectedCountryData()
                                .dialCode;
                            const mobile = phoneInputModal.getNumber();
                            $(this).val(mobile);
