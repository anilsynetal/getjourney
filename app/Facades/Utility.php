...
class Utility
{
    ...
    public function getsettings($value = '')
    {
        ...
        if (!empty(auth()->user()->id)) {
            $setting->where('created_by', auth()->user()->id);
        } else {
            $setting->whereNull('created_by');
        }
    ...
    {
        $row = Setting::where('key', $formatted_array['key'])->where('created_by', auth()->user()->id)->first();
        if (empty($row)) {
            $formatted_array['created_by'] = auth()->user()->id;
            $formatted_array['created_by_ip'] = request()->ip();
            Setting::create($formatted_array);
        } else {
            $formatted_array['updated_by'] = auth()->user()->id;
            $formatted_array['updated_by_ip'] = request()->ip();
            $row->update($formatted_array);
        }
        $affected_row = Setting::where('key', $formatted_array['key'])->where('created_by', auth()->user()->id)->first();
        return $affected_row;
    }

    public function getpath($name)
    {
        if (config('filesystems.default') == 'local'  && auth()->user()->id == null) {
